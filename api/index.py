...
ACTIONS = {
    "ip_info", "pincode", "ifsc_info", "gst_info", "email_info",
    "url_info", "dns_info", "domain_info", "phone_validate",
    "username_check", "headers_info", "port_check", "reverse_dns",
    "asn_info", "timezone_info", "text_extract",
}
...
async def get_json(url: str, **kwargs: Any) -> Any:
    async with httpx.AsyncClient(
        ...
        headers={"User-Agent": "HiTech-Free-API/1.0", "Accept": "application/json"},
    ) as client:
...
async def head_request(url: str) -> dict[str, Any]:
    async with httpx.AsyncClient(
        timeout=TIMEOUT,
        follow_redirects=True,
        headers={"User-Agent": "HiTech-Free-API/1.0"},
    ) as client:
...
async def gst_info(q: str) -> dict[str, Any]:
    gst = q.upper().strip()
    if not re.fullmatch(r"\d{2}[A-Z]{5}\d{4}[A-Z][A-Z0-9]Z[A-Z0-9]", gst):
    ...
    return {
        "gstin": gst,
        "state_code": gst[:2],
        "state": states.get(gst[:2], "Unknown"),
        "pan_segment": gst[2:12],
...
async def headers_info(q: str) -> dict[str, Any]:
    ...
    return {
        ...
        "security_headers": {k: v for k, v in result["headers"].items() if k.lower() in wanted},
    }
...
async def phone_validate(q: str) -> dict[str, Any]:
    ...
    return {
        ...
        "note": "Validation only; no owner, address, Aadhaar or private-record lookup.",
    }
...
HANDLERS = {
    "ip_info": ip_info, "pincode": pincode, "ifsc_info": ifsc_info,
    "gst_info": gst_info, "email_info": email_info, "url_info": url_info,
    "dns_info": dns_info, "domain_info": domain_info, "phone_validate": phone_validate,
    "username_check": username_check, "headers_info": headers_info, "port_check": port_check,
    "reverse_dns": reverse_dns, "asn_info": asn_info, "timezone_info": timezone_info,
